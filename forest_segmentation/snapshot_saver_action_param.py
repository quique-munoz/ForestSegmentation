...
Result: (igual que versión previa)
  bool success
  string error
  string output_dir
  string basename
...
Notas:
- No depende de message_filters; usa *cachés* por tópico para casar por tiempo.
- Si una modalidad no se solicita, su ruta en el Result va "" (vacía).
- Si se solicita CameraInfo y no hay aún, no bloquea (guarda el resto igual).
- Parámetros de ROS 2 para personalizar topics y frames.
"""
...
class SnapshotSaverAction(Node):
    def __init__(self):
        ...
        self.declare_parameter('base_snap_dir', '')  # si vacío, se resuelve al estilo del proyecto
    ...
    def _closest_in_slop(self, cache: deque, t_anchor: float, slop: float):
        """Devuelve el msg más cercano a t_anchor si |Δt|<=slop, si no None."""
        best = None
        best_dt = None
        ...
        if self._require_lidar and lidar_msg is not None:
            try:
                if len(lidar_msg.data) > 0 and lidar_msg.point_step > 0:
                    pts = np.frombuffer(lidar_msg.data, dtype=np.float32)
                    pts = pts.reshape(-1, int(lidar_msg.point_step / 4))[:, :3]
            ...
            # TF camera<-lidar (opcional)
            try:
                tf = self.tf_buffer.lookup_transform(
                    self.camera_frame, self.lidar_frame, Time(), timeout=Duration(seconds=0.5)
                )
    ...
    async def _execute_cb(self, goal_handle):
        ...
        with self._goal_lock:
            ...
            if not (self._require_image or self._require_gps or self._require_lidar):
                goal_handle.abort()
                return TakeSnapshot.Result(success=False, error='No modalities requested',
        ...
        if not self._goal_event.wait(timeout=self._timeout_sec):
            with self._goal_lock:
                self._goal_in_progress = False
            goal_handle.abort()
            return TakeSnapshot.Result(success=False, error='Timeout waiting for requested modalities',
                                       output_dir='', basename='', image_path='', points_path='', gps_path='', tf_path='')
