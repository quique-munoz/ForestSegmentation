...
# Filtrar solo puntos con alturas válidas
heat_data = []
for p in points_data["combined"]:
    if np.isfinite(p.get("height", np.nan)):
        # Intensidad = altura vegetación (limitada para evitar saturación)
        h_val = min(p["height"], 3.0)  # limita máximo visible a 3 m
        heat_data.append([p["lat"], p["lon"], h_val])

if len(heat_data) > 0:
    print(f"🔥 Añadiendo capa de calor con {len(heat_data)} puntos válidos.")
    HeatMap(
        heat_data,
        name="Densidad de vegetación",
        min_opacity=0.4,
        max_opacity=0.9,
        radius=25,           # radio en píxeles (ajusta según dispersión GPS)
        blur=15,
        gradient={
            0.0: 'green',     # vegetación baja
            0.5: 'yellow',
            1.0: 'red'        # vegetación alta
        },
    ).add_to(m)
else:
