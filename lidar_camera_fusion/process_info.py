...
CHECKPOINT_PATH = '/home/enrique/ros2_ws/src/lidar_camera_fusion/runs/checkpoint_best_ema.pth'
...
def run_inference(model, image, proc_dir):
    ...
    # === Ordenar detecciones por confianza ===
    scores = np.array(detections.confidence)
    order = np.argsort(scores)  # de menor a mayor → las más confiables se pintan al final
    ...
    # Crea un mapa RGB con los colores de cada clase
    color_mask = np.zeros_like(image, dtype=np.uint8)
    for class_id in np.unique(pred_resized):
        color = palette.by_idx(class_id).as_bgr()  # OpenCV usa BGR
        color_mask[pred_resized == class_id] = color

    # Combina imagen + máscara con transparencia
    overlay = cv2.addWeighted(image, 1 - 0.5, color_mask, 0.5, 0)
    ...
    # Crear máscara binaria de vegetación baja
    mask_smallveg = (pred_resized == SMALLVEG_CLASS)

    # Convertir a float y separar canales
    arr = image.astype(float)
    G, R, B = arr[:, :, 1], arr[:, :, 0], arr[:, :, 2]

    # Calcular VARI solo en zonas de vegetación
    vari = np.full_like(G, np.nan, dtype=float)
    m = mask_smallveg
    vari[m] = (G[m] - R[m]) / (G[m] + R[m] - B[m] + 1e-6)

    # Limitar valores a [-1, 1] y calcular percentiles para escala visual
    ...
    save_path = proc_dir / "vari_smallveg.png"
    fig.savefig(save_path, bbox_inches="tight")
    plt.close(fig)

    print(f"✅ VARI (solo vegetación) guardado en: {save_path}")
...
def project_lidar_to_camera(pred_resized, points, tf, K, proc_dir):
    ...
    # mask_lidar_smallveg = (pred_resized == SMALLVEG_CLASS).astype(np.uint8) * 255
    # cv2.imwrite(str(proc_dir / "smallveg_mask_lidar.png"), mask_lidar_smallveg)
